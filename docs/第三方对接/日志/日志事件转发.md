...
标准化日志输出接口是所有网络设备或业务系统必须具备的基本功能。通过iOA系统内日志转发功能，用户可以将iOA的日志转发至第三方日志管理或分析系统进行集中的日志管理和分析。
...
##### 1)功能路径：系统通用设置->第三方对接->日志转发->syslog服务器

![image](https://user-images.githubusercontent.com/129037288/230305279-0f5a877e-1319-4fc8-ac33-a9dbfa91b9ca.png)![image](https://user-images.githubusercontent.com/129037288/230305320-9b5fb682-f3d7-4bb4-b003-c26ba58ea01f.png)


##### 2)功能介绍

![image](https://user-images.githubusercontent.com/129037288/230310286-4c0a0eec-52e1-46a2-93b7-0b6446948a88.png)


--1、查询模块

在查询时可以根据服务器地址或者服务器名称进行模糊查询、也可以根据协议类型进行查询（目前协议类型有TCP和UDP两种）

--2、添加模块

![image](https://user-images.githubusercontent.com/129037288/230316919-cdca271a-6987-42ec-8924-5297698609a8.png)


服务器名称：任意填写（最大32个字只支持中文、英文、数字及下划线）

服务器地址：支持IP和域名的切换（最大300个字只支持常规ip和域名）

协议类型：TCP和UDP，根据实际情况选择

协议端口：1~65535

设备/Facility：支持local0~local7

服务器描述：任意（最大255个字只支持中文、英文、数字及下划线）

--ip的校验正则为：/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/

--域名的校验正则为：/[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\.?/

3、信息展示模块

在列表中展示了添加时间、服务器名称、服务器地址类型、服务器地址、协议类型、设备/Facility、服务器描述这些内容。可以进行添加、编辑和删除操作
...
##### 1)功能位置：系统设置->第三方对接->日志转发->转发策略

##### 2)功能介绍

![image](https://user-images.githubusercontent.com/129037288/230317280-49c84209-eeff-4603-bb43-0024b927e623.png)


--1、查询模块

在查询时可以根据策略名称进行模糊查询进行查询、也可以根据转发日志类型进行查询

--2、添加模块

![image](https://user-images.githubusercontent.com/129037288/230317380-07946164-0159-46f7-bdb1-5872956ca77a.png)

策略名称：任意（最大32个字只支持中文、英文、数字及下划线）
转发日志类型：全部日志
转发方式:目前只支持syslog、nsq、kafka和http四种方式
转发服务器:只能选择在服务器列表中配置的服务名
转发策略描述:任意（最大255个字只支持中文、英文、数字及下划线）
